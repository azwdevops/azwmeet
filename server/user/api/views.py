...
# celery tasks
from appemail.tasks import send_azwmeet_user_activation_email_task, send_azwmeet_password_reset_email_task


User = get_user_model()

# when calling tasks, remember to use queue=queue_name, by default mostly we use name of the project
# thus replace azwmeet with project name
...
def register_user(request):
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            send_azwmeet_user_activation_email_task.apply_async(queue='azwmeet', args=[], kwargs={**user_kwargs})
...
def user_request_password_reset(request):
    ...
    send_azwmeet_password_reset_email_task.apply_async(queue='azwmeet', args=[], kwargs={**user_kwargs})
